...
exports.createCampaignOrder = async (req, res) => {
  ...
  try {
    ...
    const insertBill =
      "INSERT INTO bills (provider, campaign_id, amount, status, user_id, bill_type, campaign_order_id) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *";
    const bill = await insertQuery(insertBill, [
      billDetail.provider,
      billDetail.campaign_id,
      billDetail.amount,
      billDetail.status,
      billDetail.user_id,
      billDetail.bill_type,
      campaign_order.rows[0].id,
    ]);
    ...
    await pool.query(
      "UPDATE bills SET invoiceno = $1, response = $2, deeplinks = $3 WHERE id = $4",
      [
...
exports.checkInvoice = async (req, res) => {
  ...
  if (result && result.rows.length > 0) {
    ...
    res.status(200).json({
      ...
      payment_status: check_invoice.data.rows[0]
        ? check_invoice.data.rows[0].payment_status
        : "pending",
